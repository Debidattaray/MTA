...
# Source the variables.txt file that contains the domainname and ips variables
source /apps/variables.txt
random_username=$(openssl rand -hex 10)
random_password=$(openssl rand -hex 10)
random_subdomain=$(openssl rand -hex 5)
echo "RDNS:: Name =  $random_subdomain , Value = $ip" >> ./DNS_UPDATE.txt
echo "SMTP:: username =  $random_username , Password = $random_password" >> ./DNS_UPDATE.txt


printf "http-access 0/0 monitor
...
<virtual-mta sharedserver.pool>
        ...
        #smtp-source-host $ip $random_subdomain.$domainname
</virtual-mta>
